...
class PositionalEmbedding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        self.pe = self.generate_positional_encoding(d_model, seq_len)

    def generate_positional_encoding(self, d_model, seq_len):
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        ...
        v = self.w_v(v).view(v.size(0), -1, self.h, self.d_k).transpose(1, 2)
...
class EncoderBlock(nn.Module):
    def __init__(self, self_attention_block: MultiHeadAttentionBlock, feed_forward_block: FeedForwardBlock, dropout: float):
        ...
        self.residual_connections = nn.ModuleList([ResidualConnection(self_attention_block.d_model, dropout),
                                                   ResidualConnection(feed_forward_block.d_model, dropout)])
...
class DecoderBlock(nn.Module):
    def __init__(self, self_attention_block: MultiHeadAttentionBlock, cross_attention_block: MultiHeadAttentionBlock, feed_forward_block: FeedForwardBlock, dropout: float):
        ...
        self.feed_forward_block = feed_forward_block
        self.residual_connections = nn.ModuleList([ResidualConnection(self_attention_block.d_model, dropout),
                                                   ResidualConnection(cross_attention_block.d_model, dropout),
                                                   ResidualConnection(feed_forward_block.d_model, dropout)])
